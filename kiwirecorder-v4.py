...
def setup_csv_logger(options):
    ...
    #  Haal scan-tag uit YAML-bestandsnaam
    if options.scan_yaml_file:
        yaml_base = os.path.basename(options.scan_yaml_file)
        tag = yaml_base.replace("scan_freqs_", "").replace(".yaml", "")
    else:
        tag = "default"

    def get_log_file_path(base_drive_folder=r"D:\RSSI data"):
        now = datetime.utcnow()
        folder = now.strftime(base_drive_folder + r"\%Y-%m-%d")
        os.makedirs(folder, exist_ok=True)
        filename = f"scan_{tag}_{now.strftime('%m-%d-%H')}.csv"
        return os.path.join(folder, filename)

    class HourlyCSVFileHandler(logging.Handler):
        def __init__(self):
            super().__init__()
            self.current_hour = None
            self.streams = []
            self.current_paths = []
            self.secondary_base = r"C:\RSSI data"
            self.fallback_base = r"C:\RSSI_fallback"  # change if you want

        def emit(self, record):
            hour_str = datetime.utcnow().strftime("%Y-%m-%d-%H")
            if hour_str != self.current_hour or not self.streams:
                self._rotate_file(hour_str)
            ...
            try:
                ...
                # Only recover on Errno 22 (EINVAL). Re-raise others so you still see real bugs.
                if getattr(e, "errno", None) != errno.EINVAL:
                    raise

            # Recovery attempt: brief pause + reopen same hourly file + retry once
            try:
                time.sleep(0.2)
            ...
            # Last resort: switch to local fallback and keep going
            try:
                self._switch_to_fallback(hour_str)
                for s in self.streams:
                    s.write(msg + "\n")
                    s.flush()
            except Exception:
                # Don’t crash the whole program because logging failed
                self.handleError(record)

        def _open_stream(self, path):
            # newline="" avoids extra blank lines on Windows in CSV-ish files
            return open(path, "a", encoding="utf-8", newline="")

        def _rotate_file(self, hour_str):
            for s in self.streams:
                try:
                    s.close()
                except Exception:
            ...
            path_primary = get_log_file_path(r"D:\RSSI data")
            path_secundary = get_log_file_path(self.secondary_base)

            self.current_paths = [path_primary, path_secundary]
            self.streams = [self._open_stream(path_primary), self._open_stream(path_secundary)]
            #if self.stream:
            #   try:
            #        self.stream.close()
            #    except Exception:
            ...
            #self.current_hour = hour_str
            #self.current_path = get_log_file_path(r"D:\RSSI data")
            #self.stream = self._open_stream(self.current_path)
        ...
        def _reopen_current(self):
            ...
            self.streams = [self._open_stream(p) for p in self.current_paths]
            
            
            # Reopen whatever file we *think* we’re currently writing
            #if self.stream:
            #    try:
            #        self.stream.close()
            #    except Exception:
            #        pass
            # If current_path is missing for some reason, rebuild it
            #if not self.current_path:
            #    self.current_path = get_log_file_path(r"D:\RSSI data")
            #self.stream = self._open_stream(self.current_path)

        def _switch_to_fallback(self, hour_str):
            # Close current stream and redirect to local disk
            if self.stream:
                try:
                    self.stream.close()
                except Exception:
            ...
            # Mirror the same day/hour filename into C: fallback
            self.current_hour = hour_str
            self.current_path = get_log_file_path(self.fallback_base)
            self.stream = self._open_stream(self.current_path)
...
class KiwiSoundRecorder(KiwiSDRStream):
    def __init__(self, options):
        ...
        self.csv_logger = setup_csv_logger(options) # options toegevoegd binnen de haakjes
    ...
    def _squelch_status(self, seq, rssi, fmt):
        ...
        if self._squelch and type(self._squelch) == list: ## scan mode
            ...
            self._need_nl = True
            sys.stdout.flush()
            # self.csv_logger.info(f"{t},{self._freq},{self._options.scan_state},{rssi}") vervangen door onderstaande om WAIT niet te loggen
            if self._options.scan_state != 'WAIT':
                self.csv_logger.info(f"{t},{self._freq},{self._options.scan_state},{rssi}")
        return is_open
